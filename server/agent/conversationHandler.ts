...
export const processMessage = async (
    ...
    switch (intent.type) {
        ...
        case 'full_automation':
            return handleFullAutomation(userMessage, intent, currentState);
...
const handleFullAutomation = (
    ...
    return {
        ...
        stateUpdates: {
            ...
            autoFillFields: new Set(['primaryKeyword', 'secondaryKeywords', 'title']),
        },
...
const handlePartialInfo = (
    ...
    const updatedData = {
        ...currentState.data,
        ...(extractedData.topic && { topic: extractedData.topic }),
        ...(extractedData.primaryKeyword && { primaryKeyword: extractedData.primaryKeyword }),
        ...(extractedData.secondaryKeywords && { secondaryKeywords: extractedData.secondaryKeywords }),
    ...
    if (extractedData.primaryKeyword) capturedItems.push(`keyword: "${extractedData.primaryKeyword}"`);
    if (extractedData.secondaryKeywords) capturedItems.push(`secondary keywords: ${extractedData.secondaryKeywords.length}`);
    if (extractedData.title) capturedItems.push(`title: "${extractedData.title}"`);

    let assistantMessage = capturedItems.length > 0
    ...
    const criticalFieldsUpdated =
        (extractedData.topic && extractedData.topic !== currentState.data.topic) ||
        (extractedData.primaryKeyword && extractedData.primaryKeyword !== currentState.data.primaryKeyword);
    ...
    if (criticalFieldsUpdated) {
        console.log('🔄 [FLOW RESTART] Critical fields updated, resetting progress');
    ...
    if (extractedData.topic) {
        ...
        if (currentState.preferences?.automationLevel === 'full') {
            ...
            if (!currentState.data.referenceUrls || currentState.data.referenceUrls.length === 0) {
                ...
                shouldRunAgent = false;
            } else if (!currentState.data.interlinks || currentState.data.interlinks.length === 0) {
...
const handleRefinement = (
    ...
    const newTopic = extractedData.topic || (regexTopic.length > 3 ? regexTopic : null);
    ...
    const clearDependentFields = (modifiedField: string): Partial<AgentState> => {
    ...
    if (newTopic) {
        console.log(`🔄 [REFINEMENT] Topic change detected: "${newTopic}". Resetting flow.`);
        return {
    ...
    if (newPrimaryKeyword) {
        console.log(`🔄 [REFINEMENT] Primary keyword change: "${newPrimaryKeyword}"`);
        return {
            ...
            stateUpdates: {
                ...
                userProvidedFields: new Set([...currentState.userProvidedFields, 'primaryKeyword']),
                halt: null,
            },
            shouldRunAgent: true,
        };
    ...
    if (newSecondaryKeywords) {
        ...
        return {
            ...
            stateUpdates: {
                ...
                userProvidedFields: new Set([...currentState.userProvidedFields, 'secondaryKeywords']),
                halt: null,
            },
            shouldRunAgent: true,
        };
    ...
    const isAskingForOptions = /(?:options|suggestions|what are|give me|show me|best|good)/i.test(userMessage);
    ...
    if (newTitle && !isAskingForOptions) {
        ...
        return {
            ...
            stateUpdates: {
                ...
                userProvidedFields: new Set([...currentState.userProvidedFields, 'title']),
                halt: null,
            },
            shouldRunAgent: true,
        };
    ...
    if (modificationRequest) {
        ...
        console.log(`🔄 [REFINEMENT] User wants to modify ${modificationRequest}, jumping to ${targetStep}`);
    ...
    return {
        assistantMessage: "I'll help you refine the content. What specifically would you like to improve?",
        stateUpdates: {},
        shouldRunAgent: false,
    };
};
...
const handleApproval = (currentState: AgentState): ConversationResponse => {
    ...
    if (currentStep === 'outline_confirmation' || currentState.halt?.reason === 'await_outline_start_confirmation') {
...
const handleSkipStep = (currentState: AgentState): ConversationResponse => {
    ...
    if (currentStep === 'references') {
        ...
        return {
            assistantMessage: "Skipped. Please add any internal links to your existing content (optional).",
            stateUpdates: {
