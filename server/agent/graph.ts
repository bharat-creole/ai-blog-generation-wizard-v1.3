...
const route = (state: AgentState) => {
    ...
    switch (state.currentStep) {
        ...
        case 'title':
            ...
            console.log(`   🔍 Automation Level: ${state.preferences?.automationLevel}`);
            ...
            console.log(`   → Route: __end__ (title set, awaiting next step)`);
            return '__end__';

        case 'references':
            // Optional step - halt for user input
            // User can provide references or skip
            console.log(`   → Route: __end__ (awaiting references input)`);
            return '__end__';

        case 'interlinking':
            // Optional step - halt for user input
            // User can provide interlinks or skip
            console.log(`   → Route: __end__ (awaiting interlinking input)`);
            return '__end__';

        case 'outline_confirmation':
            // Halt to ask user if they want to proceed to outline generation
            console.log(`   → Route: __end__ (awaiting outline confirmation)`);
            return '__end__';
        ...
        case 'outline':
            ...
            if (!state.outlineApproved) {
                console.log(`   → Route: __end__ (awaiting outline approval)`);
                return '__end__';
            }
        ...
        default:
            ...
            console.log(`   ⚠️  Unknown step: ${state.currentStep}, defaulting to topic`);
            return '__end__';
    }
};
