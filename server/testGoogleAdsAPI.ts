...
const testGoogleAdsAPI = async (): Promise<void> => {
	const testUrl =
		process.env.GOOGLE_ADS_URL ||
		'https://tutorials.botsfloor.com/ai-agents-vs-agentic-ai-whats-the-difference-and-why-does-it-matter-03159ee8c2b4'; // Change this to test different URLs
	...
	try {
		...
		if (response.ok && data.results) {
			console.log(
				`✅ Success! Found ${data.results.length} keyword ideas:\n`
			);
			...
			data.results.slice(0, 5).forEach((result, index) => {
				...
				console.log(
					`   - Avg Monthly Searches: ${
						metrics.avgMonthlySearches || 'N/A'
					}`
				);
				console.log(
					`   - Competition: ${
						metrics.competition || 'N/A'
					}`
				);
				console.log(
					`   - Low Top Page Bid: ${
