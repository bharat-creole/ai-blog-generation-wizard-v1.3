...
import { selectReferences, toVirtualFiles, SelectedReference } from '../retrieval';
...
const mergeFiles = (original: ReferenceFile[], virtuals: ReferenceFile[]): ReferenceFile[] => {
  // Avoid duplicating by name+mime
  const key = (f: ReferenceFile) => `${f.name}|${f.mimeType}|${f.base64.length}`;
...
async function stepExtractHeadings(state: WizardState): Promise<WizardState> {
  const { data } = state;
  if (!data.referenceFiles || data.referenceFiles.length === 0) return state;
  const { byFile, merged } = await extractFromFiles(data.referenceFiles);
  try { if ((import.meta as any)?.env?.DEV) { console.log('Extracted headings (merged):', merged); console.log('Extracted headings by file:', byFile); } } catch {}
  return {
    ...state,
    data: {
      ...data,
...
async function stepPlanOutline(state: WizardState): Promise<WizardState> {
  ...
  const virtuals = selectedReferences ? toVirtualFiles(selectedReferences) : [];
  const workingData: BlogData = {
    ...data,
    referenceFiles: mergeFiles(data.referenceFiles || [], virtuals),
  };
  const outline = await geminiService.generateOutline(workingData, workingData.apiKey, feedback);
  return { ...state, outline };
}

export async function invokeOutline(data: BlogData, feedback?: string): Promise<{ outline: OutlineSection[]; data: BlogData }> {
  ...
  try {
    ...
    const extractNode = async (s: any) => {
      ...
      return { data: ns.data, extractedHeadings: ns.extractedHeadings } as any;
    };
    ...
    const result = await app.invoke({ data, feedback, extractedHeadings: undefined, selectedReferences: undefined, outline: undefined, error: undefined });
...
export async function invokeBlog(data: BlogData): Promise<{ content: string; data: BlogData }> {
  ...
  try {
    ...
    const writeNode = async (s: any) => {
      const content = await geminiService.generateBlogPost(s.data, s.data.outline || [], s.data.apiKey);
    ...
    // Fallback: custom orchestrator
    const content = await geminiService.generateBlogPost(data, data.outline || [], data.apiKey);
